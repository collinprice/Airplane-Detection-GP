...
public class SimpleStatistics extends Statistics implements SteadyStateStatisticsForm //, ec.eval.ProvidesBestSoFar
    ...
    public void postEvaluationStatistics(final EvolutionState state)
        ...
        String temp = best_i[0].fitness.fitnessToStringForHumans();
    	int start = temp.indexOf('=');
    	int end = temp.indexOf('.');
    	int misses = Integer.parseInt(temp.substring(start+1, end));
        state.output.print("" + misses, statisticslog);
        state.output.print(",", statisticslog);
        
        int total = 0;
        Individual[] inds = state.population.subpops[0].individuals;
        for (int i = 0; i < inds.length; i++) {
        	temp = inds[i].fitness.fitnessToStringForHumans();
        	start = temp.indexOf('=');
        	end = temp.indexOf('.');
        	misses = Integer.parseInt(temp.substring(start+1, end));
        	total += misses;
        }
        state.output.print("" + (total / inds.length), statisticslog);
        state.output.print(",", statisticslog);
        
        temp = best_i[0].fitness.fitnessToStringForHumans();
    	start = temp.indexOf("Hits=");
    	misses = Integer.parseInt(temp.substring(start+5, temp.length()));
